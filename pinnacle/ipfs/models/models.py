...
class PinMeta(BaseModel):
    ...
    @classmethod
    def from_model(cls, model: BaseModel, exclude: set[str]):
        _dict = model.dict(exclude=exclude, exclude_none=True)
        _data = {k: str(v) for k, v in _dict.items()}

        return cls(__root__=_data)
